...
export const getStoryViewers = async (req, res) => {
  try {
    ...
    let filtered = story.views.filter(
      (v) => v.user?._id.toString() !== story.user.toString()
    );
    ...
    const uniqueViews = Array.from(uniqueMap.values());

    uniqueViews.sort((a, b) => new Date(b.viewedAt) - new Date(a.viewedAt));

    res.json(uniqueViews);
  } catch (error) {
    console.log("🔥 ERROR IN getStoryViewers:", error);
