...
const StoriesCarousel = ({
  ...
  const getActualViewerCount = () => {
    ...
    const filteredViews = currentStory.views.filter(
      (view) => view._id !== currentUserId
    );
    return filteredViews.length;
  };

  const actualViewerCount = getActualViewerCount();
  ...
  const loadViewers = async () => {
    ...
    try {
      ...
      const filteredViewers = viewers.filter(
        (viewer) => viewer._id !== currentUserId
      );
      ...
      <div className="fixed inset-0 bg-black z-50">
        ...
        <div className="absolute top-8 left-4 right-4 flex items-center justify-between z-20">
          ...
          <div className="flex items-center gap-2">
            {isStoryOwner && actualViewerCount > 0 && (
              <button
